...
def _render_case_group(case_name: str, cases: list[dict[str, Any]]) -> str:
    ...
    for target in targets:
        target_run = next(
            case for case in cases if case.get("target") == target
        )
        target_sections.append(
            f"<h3>{_escape(target.title())}</h3>"
            f'<div class="target-run">{_render_run(target_run)}</div>'
        )
    return f"""
<section class="case">
  ...
  <details>
    <summary>Fixture files visible to the Skill run</summary>
...
.target-run {{ display: grid; grid-template-columns: minmax(0, 1fr); gap: 1rem; align-items: start; }}
...
<div class="notice">
  ...
  A successful process only proves that the model command completed. Inspect
  each target's final response and tool trace, then record a specific PASS or
  FAIL reason in grading.json.
</div>
<table>
<thead><tr><th>Recorded assertions</th><th>Recorded pass rate</th><th>Review states</th><th>Mean tokens</th><th>Mean ms</th></tr></thead>
